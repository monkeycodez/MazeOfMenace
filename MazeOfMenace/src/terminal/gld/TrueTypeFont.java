...
public class TrueTypeFont {
	...
	private void createSet(char[] customCharsArray) {
		// If there are custom chars then I expand the font texture
		// twice
		if (customCharsArray != null && customCharsArray.length > 0) {
			textureWidth *= 2;
		}

		// In any case this should be done in other way. Texture with
		// size 512x512
		// can maintain only 256 characters with resolution of 32x32.
		// The texture
		// size should be calculated dynamicaly by looking at character
		// sizes.
		...
		try {
			...
			for (int i = 0; i < 256 + customCharsLength; i++) {
				...
				// get 0-255 characters and then custom
				// characters
				char ch = (i < 256) ? (char) i
				...
				if (i < 256) { // standard characters
					...
					customChars.put(new Character(ch),
							newIntObject);
				}
	...
	private void drawQuad(float drawX, float drawY, float drawX2,
			float drawY2, float srcX, float srcY, float srcX2,
			float srcY2) {
	...
	public void drawString(float x, float y, String whatchars,
		...
		drawString(x, y, whatchars, 0, whatchars.length() - 1, scaleX,
				scaleY, ALIGN_LEFT, null);
	}
	...
	public void drawString(float x, float y, String whatchars,
			int startIndex, int endIndex, float scaleX,
		...
		boolean usecol = colarr != null
				&& colarr.length == whatchars.length();
	...
	public static int loadImage(BufferedImage bufferedImage) {
		try {
			...
			if (db instanceof DataBufferInt) {
				int intI[] = ((DataBufferInt) (bufferedImage
				...
				byteBuffer = ByteBuffer
						.allocateDirect(width * height
								* (bpp / 8))
						.order(ByteOrder.nativeOrder())
						.put(((DataBufferByte) (bufferedImage
								.getData()
								.getDataBuffer()))
								.getData());
			}
